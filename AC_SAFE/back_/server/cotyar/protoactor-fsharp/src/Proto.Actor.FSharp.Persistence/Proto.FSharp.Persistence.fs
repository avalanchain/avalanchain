...
module Persistence = 
    ...
    type SenderInfo<'Event> = { // Mostly for streaming support
        Address: string
        Tell: 'Event -> unit
    } with static member FromPID (pid: PID) = { Address = pid.Address; Tell = fun e -> 
                                                                                pid <! e }

    type CommandProcessor<'Command,'Event,'State,'CommandError> = 
        'State -> int64 -> 'Command -> Async<Result<('Event option) * bool, 'CommandError>>
    ...
    let private systemHandler (persistence: Persistence) (_: IContext) sm = 
        match sm with 
        | Started _ -> async { do! persistence.RecoverStateAsync() |> Async.AwaitTask }
        | _ -> async {()}

    let private handler (persistence: Persistence) 
                        (processCommand: CommandProcessor<'Command, 'Event, 'State, 'CommandError>) 
                        (state: 'State) 
                        (ctx: IContext) 
                        (cmd: 'Command): Async<unit> = async { 
        let! res = processCommand state persistence.Index cmd
        match res with
        | Ok (evtOpt, save) -> match evtOpt with 
                                | Some evt ->   if save then 
                                                    do! persistence.PersistEventAsync evt |> Async.AwaitTask 
                                                if isNull(ctx.Sender) |> not then ctx.Sender <! Ok evt
                                | None -> () 
        | Error e -> e >! ctx.Sender 
    }

    let simpleProcessCommand = fun _ _ cmd -> async { return (Some cmd, true) |> Ok }

    [<RequireQualifiedAccess>]
    module CommandSourcingAndSnapshotting = 
        let persistDetailed<'Command, 'Event, 'State, 'CommandError> 
            ...
            Actor.create2Async (handler persistence processCommand state)

        let persist<'Command, 'Event, 'State, 'CommandError>
            (provider: IProvider) 
            (processCommand: CommandProcessor<'Command, 'Event, 'State, 'CommandError>)
    ...
    module Snapshotting = 
        let persist 
            ...
            Actor.create2Async (handler persistence simpleProcessCommand None)
