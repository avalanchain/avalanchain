...
val system1 = ActorSystem("test-akka-sys", ConfigFactory.parseString(config))
...
val queries = PersistenceQuery(system1).readJournalFor[LeveldbReadJournal](
...
events.runWith(Sink.foreach(x => println(s"query - ${x}")))
events.runWith(Sink.foreach(x => println(s"query1 - ${x}")))

//def createQuery[TS](filterF: TS => Boolean, )