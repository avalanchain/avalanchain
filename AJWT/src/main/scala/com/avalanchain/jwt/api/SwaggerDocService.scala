...
class SwaggerDocService(system: ActorSystem, httpInterface: String, httpPort: Int) extends SwaggerHttpService with HasActorSystem {
  ...
  override val apiTypes = Seq(ru.typeOf[NodeService], ru.typeOf[ChainService], ru.typeOf[AdminService], ru.typeOf[UsersService], ru.typeOf[ChatService], ru.typeOf[CurrencyService])
