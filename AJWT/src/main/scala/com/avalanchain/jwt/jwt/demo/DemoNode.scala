...
class DemoNode(chainNode: ChainNode)(implicit encoder: Encoder[ChainDef], decoder: Decoder[ChainDef]) {
  private implicit val materializer = chainNode.materializer

//  import scala.concurrent.ExecutionContext.Implicits.global
//
//  val tickerChainRef = ChainRef(chainNode.newChain().chainDefToken)
//  def tickerSink() = cnf.sink(tickerChainRef).toOption.get // TODO: Handle errors
////  private val ticker = Future {
////    val tickerSink = cnf.sink(tickerChainRef).toOption.get // TODO: Handle errors
...
//
//  def tickerSource(from: Position = 0, to: Position = Int.MaxValue) = chainNode.source(tickerChainRef, from, to)
//  //def yahooSource =
}
object DemoNode {
